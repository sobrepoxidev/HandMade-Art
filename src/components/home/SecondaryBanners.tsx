...
export default function SecondaryBanners({ locale }: SecondaryBannersProps) {
  return (
    <div className="mx-auto max-w-[1500px]">
      ...
      <Carousel>
        <BannerTemplate linkHref="/products" bgColor="bg-[#0F0C0A]">
          <Image
            src="/taller/cat-cocina.webp"
            alt=""
            fill
          ...
          <div className="relative flex h-full items-center px-6 sm:px-10 lg:px-16">
            <div className="max-w-xl">
              <p className="mb-3 font-sans text-[10px] uppercase tracking-[0.22em] text-[#E0A83A] sm:text-[11px]">
                {locale === 'es' ? 'Hecho a mano - Costa Rica' : 'Handmade - Costa Rica'}
              </p>
              <h3 className="font-display text-2xl font-medium leading-[1.1] tracking-[-0.005em] text-[#F1E7D6] sm:text-3xl lg:text-[40px]">
                {locale === 'es'
                  ? 'Cada pieza, una historia tallada en madera.'
                  : 'Every piece, a story carved in wood.'}
              </h3>
              <p className="mt-3 hidden max-w-md text-[13px] leading-relaxed text-[#C9BBA5] sm:block lg:mt-4 lg:text-[14.5px]">
                {locale === 'es'
              ...
              <span
                className="mt-4 inline-flex min-h-[44px] items-center gap-2 rounded-sm bg-[#E0A83A] px-5 py-2.5 text-[13px] font-semibold tracking-wide text-[#161210] transition-colors duration-200 lg:mt-6"
                aria-hidden
              >
        ...
        <BannerTemplate linkHref="/shipping" bgColor="bg-[#161210]">
          <div className="relative flex h-full items-center justify-between gap-6 px-6 sm:px-10 lg:gap-12 lg:px-16">
            <div className="max-w-xl flex-1">
              <p className="mb-3 inline-flex items-center gap-1.5 font-sans text-[10px] uppercase tracking-[0.18em] text-[#F3C56B] sm:text-[11px]">
                <Truck className="h-3 w-3" strokeWidth={1.75} aria-hidden />
                {locale === 'es' ? 'Envíos en todo el país' : 'Nationwide shipping'}
              </p>
              <h3 className="font-display text-2xl font-medium leading-[1.15] tracking-[-0.005em] text-[#F1E7D6] sm:text-3xl lg:text-[36px]">
                {locale === 'es'
                  ? 'Llevamos tu pieza a cualquier parte de Costa Rica.'
                  : 'We ship every piece anywhere in Costa Rica.'}
              </h3>
              <p className="mt-2 text-[14px] text-[#C9BBA5] lg:mt-3 lg:text-base">
                {locale === 'es' ? (
                  <>
                    <span className="font-display text-lg font-semibold text-[#F3C56B] lg:text-xl">Desde $6.99 USD</span>
                    <span className="hidden sm:inline">{' - '}variable según peso</span>
                  </>
                ) : (
                  <>
                    <span className="font-display text-lg font-semibold text-[#F3C56B] lg:text-xl">From $6.99 USD</span>
                    <span className="hidden sm:inline">{' - '}weight-based</span>
                  </>
                )}
              </p>
              <span
                className="mt-4 inline-flex min-h-[44px] items-center gap-2 rounded-sm bg-[#E0A83A] px-5 py-2.5 text-[13px] font-bold tracking-wide text-[#161210] transition-colors duration-200 hover:bg-[#F3C56B] lg:mt-5"
                aria-hidden
              >
        ...
        <BannerTemplate linkHref="/reinsercion-sociolaboral" bgColor="bg-[#0F0C0A]">
          <Image
          ...
          <div className="relative flex h-full items-center px-6 sm:px-10 lg:px-16">
            <div className="max-w-xl">
              ...
              <span
                className="mt-4 inline-flex min-h-[44px] items-center gap-2 rounded-sm border border-[#F1E7D6]/40 bg-transparent px-5 py-2.5 text-[13px] font-semibold tracking-wide text-[#F1E7D6] transition-colors duration-200 lg:mt-6"
                aria-hidden
              >
