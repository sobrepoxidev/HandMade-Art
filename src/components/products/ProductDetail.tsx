...
export default function ProductDetail({
  ...
  // Live announcement for screen readers when something happens (list add / share).
  ...
  // Add to the quote-request list (replaces the old add-to-cart flow)
  const handleAddToList = () => {
    ...
    const productName =
      ...
      product.name ||
      '';
    const firstImage = isMediaArray(product.media)
      ? product.media[0].url
      : undefined;
    ...
    interestList.addItem(
      {
        ...
        dolar_price: Number(product.dolar_price) || 0,
    ...
    notify.success(
      locale === 'es'
        ? `${productName} · ${quantity} ${quantity === 1 ? 'unidad' : 'unidades'} en tu lista de cotización`
        : `${productName} · ${quantity} ${quantity === 1 ? 'unit' : 'units'} in your quote list`
    );

    setAnnouncement(
      locale === 'es'
        ? `Pieza añadida a tu lista de cotización: ${productName}, cantidad ${quantity}.`
        : `Piece added to your quote list: ${productName}, quantity ${quantity}.`
    );
        ...
        <div className="w-full md:w-5/12 mt-6 md:mt-0">
          ...
          <div className="pt-5 pb-6 border-t border-[#3A2E24] space-y-5">
            ...
            <div className="flex flex-col gap-3">
              ...
              >
                <Send className="h-4 w-4 mr-2.5" strokeWidth={2} aria-hidden />
